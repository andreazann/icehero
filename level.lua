...
function collisioni2(event)
	...
	if (event.phase == "began") then
		...
		if(type == "meta") then
			...
			score_value = calcScore:exec(score_value, accNumber, levelVar)
			print(score_value)
			--record_value = insertScore:record_value(record_value)
			popUpEnd:init(PP_params, score_value, localGroup)
			timer.cancel(tmr)
			tmr = nil
			--insertScore:go(PP_params, score_value)
			Runtime:removeEventListener("touch", aumento)
...
function aumento(event)
	...
	if (event.phase == "ended") then
		...
		local xPin = pinguino.x

		local yPin = pinguino.y

		if (event.x < xPin) then
		...
		if (times == 1) then
			...
			if (accNumber == 0) then
				...
				touchTmr = timer.performWithDelay(700, timesAcc, -1)
			...
			pinguino:setLinearVelocity(xAcc, yAcc)
...
function popUp()
	...
	popUp_bg.isVisible = false
			...
			Runtime:addEventListener("touch", aumento)
